...
module.exports = function () {
    ...
    it('should change pages when next/prev page buttons clicked', function (done) {
        ...
        let next = el.find('ul.pagination li:contains("Next") a');
        ...
        Vue.nextTick()
            .then(() => {
                ...
                let prev = el.find('ul.pagination li:contains("Previous") a');
                assert(prev[0], 'Previous selector not found');
                prev[0].dispatchEvent(new Event('click'));
                return Vue.nextTick();
            })
            .then(() => {
                assert.equal(1, filter.page, `page not set to 1: ${filter.page}`);
            })
            .then(done, done);
    });

    it('should refresh when page buttons clicked', function (done) {
        let el = $(theNovaTable.$el);
        let filterRan = 0;
        ...
        assert.equal(filter, theNovaTable.filter);
